...
fn deltas(report: &[i64]) -> Vec<i64> {
    report.iter().enumerate().skip(1).map(|(i, x)| x - report[i - 1]).collect()
}

fn report_is_safe(report: &[i64]) -> bool {
    let deltas = deltas(report);
    let increasing = deltas.iter().all(|&d| d > 0 && d <= 3);
    let decreasing = deltas.iter().all(|&d| d < 0 && d >= -3);
...
fn report_is_safe_dampener(report: &[i64]) -> bool {
    ...
    let decreasing_count = diff.iter().filter(|&&d| d < 0 && d >= -3).count();
    if increasing_count == diff.len() || decreasing_count == diff.len() {
        ...
        let report_b = [&report[..(bad_delta_pos + 1)], &report[(bad_delta_pos + 2)..]].concat();
...
pub fn solve(input: String) -> Solution {
    ...
    // reports are safe?
    let safe_reports = reports.iter().filter(|report| report_is_safe(report)).count();
    ...
    let safe_reports_dampened = reports.iter().filter(|report| report_is_safe_dampener(report)).count();
