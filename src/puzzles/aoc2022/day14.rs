...
pub fn solve(input: String) -> Solution {
    ...
    solution.set_part_a(cave_state.sand_at_rest());

    // reset variables in between runs
    cave_state.sand = None;
    cave_state.sand_state = SandState::NotSpawned;

    // Part B: Using your scan, simulate the falling sand until the source of the sand becomes
    // blocked. How many units of sand come to rest?
    cave_state.add_floor();
    cave_state.run_to_completion();
    solution.set_part_b(cave_state.sand_at_rest());
